...
def sort_by_location(user_loc, filter_df, infocan):
    ...
    frames = []
    for loc in lst_loc:
        # calculate the distance, just give an estimate, we don't use google API here for the sake of the speed
        distance = (((user_loc[0] - infocan.loc[loc]['loc x'])/900*2)**2 + ((user_loc[1] - infocan.loc[loc]['loc y'])/636*2.5)**2)**(1/2)
